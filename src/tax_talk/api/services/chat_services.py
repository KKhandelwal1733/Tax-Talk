...
class ChatService:
    ...
    async def answer(
        ...
        *,
        ...
        with propagate_attributes(user_id=user_id) if user_id else nullcontext():
            ...
            return ChatResponse(
                answer=answer, citations=hits[: request.top_k], faithfulness=faithfulness
            )
    ...
    async def stream_answer(
        ...
        *,
        ...
        with propagate_attributes(user_id=user_id) if user_id else nullcontext():
            ...
            async for token in strategy.generate_stream_async(
                prompt=prompt, model=settings.chat_model
            ):
    ...
    @observe(
        name="api-query-rewrite", as_type="generation", capture_input=False, capture_output=False
    )
